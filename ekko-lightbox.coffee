...
EkkoLightbox = (element, options) ->
  ...
  @modal_id = if @options.markup then 'lightbox-modal' else if @options.modal_id then @options.modal_id else 'ekkoLightbox-' + Math.floor((Math.random() * 1000) + 1)
  ...
  if @lightbox_container.length == 0 then @modal_body.append '<div class="ekko-lightbox-container"><div></div></div>'
...
# instagram load their content into iframe's so this can be put straight into the element
  ...
  isExternal: (url) ->
    ...
    return true if typeof match[2] == "string" && match[2].length > 0 && match[2].replace(new RegExp(":(" + {
        "http:": 80,
        "https:": 443
